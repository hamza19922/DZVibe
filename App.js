    ...
    <View style={styles.card}>
      ...
      <View style={styles.actions}>
        {actions.map((action, index) => (
          <Pressable key={index} style={({ pressed }) => [styles.action, pressed && styles.pressed]} onPress={action.onPress} hitSlop={8}>
            <Text style={[styles.actionIcon, liked && index === 0 && styles.liked]}>{action.icon}</Text>
...
export default function App() {
  ...
  const selectSection = (name) => {
    setSection(name);
    if (name !== 'الرئيسية') Alert.alert(name, `تم فتح قسم ${name}. سنضيف محتواه الكامل في الخطوة التالية.`);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.feed}>
        {FEED.map((item, index) => <View key={item.id} style={styles.cardWrap}><FeedCard item={item} active={active === index} liked={liked} onLike={() => setLiked(value => !value)} /></View>)}
      </View>
...
const styles = StyleSheet.create({
  ...
  action: { alignItems: 'center', minWidth: 58, paddingVertical: 3 },
  ...
  navButton: { minWidth: 50, minHeight: 44, alignItems: 'center', justifyContent: 'center' },

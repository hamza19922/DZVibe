...
# The release workflow can run more than once. If recovery is already present,
# only add the email verification redirect if an older patched build lacks it.
if 'function PasswordRecovery({onDone})' in text and 'نسيت كلمة المرور؟' in text:
    if 'emailRedirectTo:Linking.createURL' not in text:
        old = "supabase.auth.signUp({email:email.trim(),password:pass,options:{data:{name:name.trim()}}})"
        new = "supabase.auth.signUp({email:email.trim(),password:pass,options:{emailRedirectTo:Linking.createURL('verify'),data:{name:name.trim()}}})"
        if old in text:

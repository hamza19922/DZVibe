...
export default function UploadVideo({ user, onDone, onCancel }) {
  ...
  const choose = async () => {
    try {
      ...
      if (!permission.granted) {
        Alert.alert('صلاحية مطلوبة', 'اسمح للتطبيق بالوصول إلى الفيديوهات من إعدادات الهاتف.');
        return;
      }
      ...
      Alert.alert('تعذر فتح المعرض', e?.message || 'حدث خطأ أثناء اختيار الفيديو.');
    }
  };

  const publish = async () => {
    if (!video?.uri) {
      Alert.alert('اختر فيديو', 'اختر فيديو من الهاتف أولاً.');
      return;
    }
    if (!user?.id) {
      Alert.alert('الجلسة غير صالحة', 'سجّل الدخول من جديد ثم حاول مرة أخرى.');
      return;
    }
    setBusy(true);
    try {
      ...
      // Supabase documents ArrayBuffer for React Native uploads; Blob/File/FormData are unreliable there.
      const arrayBuffer = await response.arrayBuffer();
      const upload = await supabase.storage.from('videos').upload(path, arrayBuffer, {
      ...
      if (!publicUrl) throw new Error('تعذر إنشاء رابط الفيديو.');
  ...
  return <View style={s.page}>
    ...
    <Pressable disabled={busy} onPress={publish} style={[s.publish, busy && { opacity: 0.6 }]}><Text style={s.publishText}>{busy ? 'جارٍ الرفع...' : 'نشر الفيديو'}</Text></Pressable>
  </View>;
}
